...
현재 공개 릴리즈는 `9.0.7-2`입니다. GHCR의 `9.0.7-2`와 `latest` 태그는 `linux/amd64`, `linux/arm64`, `linux/arm/v7`를 포함하는 multi-arch manifest입니다.
...
```bash
docker pull ghcr.io/suikano1304/kavita-gds:9.0.7-2
```
...
services:
  kavita:
    image: ghcr.io/suikano1304/kavita-gds:9.0.7-2
...
`9.0.7-2`의 기본 배포물은 GHCR multi-arch image입니다. Docker pull이 불가능한 폐쇄망 환경에서는 GHCR image를 별도 registry로 미러링하거나, 필요한 플랫폼의 OCI archive를 따로 생성해 반입하세요. 과거 릴리스의 GitHub tarball은 해당 릴리스 태그 기준 산출물입니다.

## 주요 수정

`9.0.7-2`에는 다음 안정화가 포함되어 있습니다.
...
`9.0.7-2` 배포 전 다음 검증을 수행했습니다.

- GDS metadata parser focused tests 8개 통과
- `Kavita.Services.Tests` 전체 통과: 2246 passed, 0 failed, 6 skipped
- local cover regression fixture 2회 반복 통과, SQLite `quick_check=ok`
- pushed GHCR `9.0.7-2` 이미지의 `linux/amd64` startup `/api/health` 200 확인
