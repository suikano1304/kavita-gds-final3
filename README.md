...
현재 공개 릴리즈는 `9.0.7-5`입니다. GHCR의 `9.0.7-5`와 `latest` 태그는 `linux/amd64`, `linux/arm64`, `linux/arm/v7`를 포함하는 multi-arch manifest입니다.
...
```bash
docker pull ghcr.io/suikano1304/kavita-gds:9.0.7-5
```
...
services:
  kavita:
    image: ghcr.io/suikano1304/kavita-gds:9.0.7-5
...
`9.0.7-5`의 기본 배포물은 GHCR multi-arch image입니다. Docker pull이 불가능한 폐쇄망 환경에서는 GHCR image를 별도 registry로 미러링하거나, 필요한 플랫폼의 OCI archive를 따로 생성해 반입하세요. 과거 릴리스의 GitHub tarball은 해당 릴리스 태그 기준 산출물입니다.

## 주요 수정

`9.0.7-5`에는 다음 안정화가 포함되어 있습니다.
...
`9.0.7-5` 배포 전 다음 검증을 수행했습니다.

- duplicate broken/valid EPUB row 회귀 샘플의 cold-cache `book-info`, `chapters`, `book-page`, EPUB resource API 200 확인
- `CacheServiceTests` focused regression tests 24개 통과
- GDS targeted scan follow-up focused tests 2개 통과
- GDS cover service focused tests 8개 통과
- local cover regression fixture 2회 반복 통과, SQLite `quick_check=ok`
- pushed GHCR `9.0.7-5` 이미지의 `linux/amd64` startup `/api/health` 200 확인
- `linux/arm64` image를 qemu에서 기동해 `/api/health` 200 확인
- `linux/arm/v7` image를 qemu에서 기동해 `/api/health` 200 확인
- 운영 컨테이너를 GHCR `9.0.7-5` amd64 이미지로 재기동하고 Docker health `healthy` 확인
