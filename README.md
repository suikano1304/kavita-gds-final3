...
현재 공개 릴리즈는 `9.0.7-4`입니다. GHCR의 `9.0.7-4`와 `latest` 태그는 `linux/amd64`, `linux/arm64`, `linux/arm/v7`를 포함하는 multi-arch manifest입니다.
...
- pushed GHCR `9.0.7-4` 이미지의 `linux/amd64` startup `/api/health` 200 확인
- `linux/arm64` image를 qemu에서 기동해 `/api/health` 200 확인
- `linux/arm/v7` image를 qemu에서 기동해 `/api/health` 200 확인
- 운영 컨테이너를 GHCR `9.0.7-4` amd64 이미지로 재기동하고 Docker health `healthy` 확인
