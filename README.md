...
현재 공개 릴리즈는 `9.0.7-1`입니다. GHCR의 `9.0.7-1`와 `latest` 태그는 `linux/amd64`, `linux/arm64`, `linux/arm/v7`를 포함하는 multi-arch manifest입니다.
...
```bash
docker pull ghcr.io/suikano1304/kavita-gds:9.0.7-1
```
...
services:
  kavita:
    image: ghcr.io/suikano1304/kavita-gds:9.0.7-1
...
`9.0.7-1`의 기본 배포물은 GHCR multi-arch image입니다. Docker pull이 불가능한 폐쇄망 환경에서는 GHCR image를 별도 registry로 미러링하거나, 필요한 플랫폼의 OCI archive를 따로 생성해 반입하세요. 과거 릴리스의 GitHub tarball은 해당 릴리스 태그 기준 산출물입니다.

## 주요 수정

`9.0.7-1`에는 다음 안정화가 포함되어 있습니다.
...
`9.0.7-1` 배포 전 다음 검증을 수행했습니다.

- hotfix 적용 전 local fixture와 운영 항목 refresh로 cover regression 검증
- pushed GHCR `9.0.7-1` 이미지의 `linux/amd64` startup health 확인
- health/version API, SQLite integrity, GDS read-only mount 확인
- TXT, ZIP/CBZ archive, EPUB, PDF reader/API와 cover/cache/log 회귀 확인
- synthetic single-spine EPUB fixture를 TOC page mapping 회귀 대상으로 확인
- `linux/arm64` image를 qemu에서 기동해 `/api/health` 200 및 Docker health `healthy` 확인
- `linux/arm/v7` image를 qemu에서 기동해 `/api/health` 200 및 Docker health `healthy` 확인
