...
Docker pull을 쓸 수 없는 `linux/amd64` 환경에서는 GitHub Release asset을 사용할 수 있습니다.

```bash
curl -L -o kavita-gds.tar.gz \
  https://github.com/suikano1304/Kavita-GDS/releases/download/v9.0.6-2/kavita-gds.tar.gz
```
...
```bash
curl -L -o SHA256SUMS \
  https://github.com/suikano1304/Kavita-GDS/releases/download/v9.0.6-2/SHA256SUMS
...
`0.9.0.2-5` 이후와 `9.0.6-2` 이미지는 production UI bundle을 포함합니다.
