...
현재 릴리즈는 `9.0.7-5`입니다. GHCR의 `9.0.7-5`와 `latest` 태그는 `linux/amd64`, `linux/arm64`, `linux/arm/v7`를 포함합니다.
...
`0.9.0.2-5` 이후와 `9.0.7-5` 이미지는 production UI bundle을 포함합니다.
