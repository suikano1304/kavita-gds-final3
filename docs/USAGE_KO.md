...
현재 릴리즈는 `9.0.7-6`입니다. GHCR의 `9.0.7-6`와 `latest` 태그는 `linux/amd64`, `linux/arm64`, `linux/arm/v7`를 포함합니다.
...
`9.0.7-6`는 다음 EPUB/OPDS/WebUI 문제를 완화합니다.

- OPDS multi-file archive chapter에서 각 ZIP/CBZ entry가 자기 파일의 id/link/pages/size를 사용하지 못하던 문제
- OPDS image stream에서 `saveProgress=false`가 진행률 저장을 막지 못하던 문제
...
`0.9.0.2-5` 이후와 `9.0.7-6` 이미지는 production UI bundle을 포함합니다.
