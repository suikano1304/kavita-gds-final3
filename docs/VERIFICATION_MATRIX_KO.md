...
- 공개 release `0.9.0.2-3`에는 주요 보정이 포함됐다.
- 하지만 운영 컨테이너는 아직 `local/kavita-gds:0.9.0.2-1`이므로, 운영 DB에서 `0.9.0.2-3`의 회복/cleanup 효과는 아직 검증되지 않았다.
...
| source/release/운영 일치 | 운영 이미지가 공개 release와 같은 source 기준으로 실행됨 | release/source는 일치. 운영은 아직 `0.9.0.2-1` | 미검증 |

## Postflight 기준

운영에 `0.9.0.2-3` 이상을 적용한 뒤 아래가 확인되어야 최종 완료로 볼 수 있다.
