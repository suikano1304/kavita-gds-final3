...
- UI/DTO에서 GDS chapter 표시 우선순위를 `TitleName -> Title -> Range`로 고정할지 별도 확인이 필요하다.
...
- 원본 cover가 없고 기존 `_s{id}.jpg` cache만 남은 경우, GDS folder cover 경로는 stale cache를 삭제할 수 있다.
- 따라서 “GDS가 읽기 전용이라 안전하다”와 “Kavita config cache가 안전하다”는 별개다.

개선 방향:

- config/covers는 DB와 같이 백업해야 한다.
- folder cover가 없는 경우 기존 cache 삭제가 필요한지, fallback cover 생성을 방해하지 않는지 추가 검토가 필요하다.
...
5. nested archive와 duplicate path를 자동 수정할지, 자료 정리 대상으로 둘지 결정한다.
6. UI chapter 표시 우선순위와 cover cache 삭제 조건을 별도 검토한다.
