...
1. 운영 소스와 git 소스를 동기화한다. 진행 중.
2. `ParseScannedFiles`의 changed propagation을 시리즈 단위로 고친다. 테스트 통과.
3. GDS archive 페이지 수가 0으로 남지 않게 고친다. 적용 및 추가 검증 필요.
...
6. mixed-format이 많은 라이브러리에서 강제 스캔 후 일반 스캔을 연속 실행해 처리 파일 수가 안정적으로 떨어지는지 확인한다.
