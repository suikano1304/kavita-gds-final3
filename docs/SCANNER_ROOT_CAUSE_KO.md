...
5. GDS UI 제목 표시 누락을 다음 release에 포함한다. source patch 완료, rebuild 필요.
6. nested archive와 duplicate path를 자동 수정할지, 자료 정리 대상으로 둘지 결정한다.
7. cover cache 삭제 조건을 별도 검토한다.
