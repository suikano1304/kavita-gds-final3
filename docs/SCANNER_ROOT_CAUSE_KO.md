...
| source/release 일치 | 운영 hotfix와 release source가 갈라지면 분석 근거가 깨진다. | `0.9.0.2-3` 배포 뒤 source branch에 GDS 타입 처리 보강 커밋이 추가되어, 다음 검증 대상 release가 필요하다. | 이후 변경은 source branch, release tarball, image를 같은 commit에서 다시 생성. |

현재 가장 큰 미완료 항목은 코드 작성이 아니라 최신 source 기준 release를 다시 만들고, 운영 적용 후 postflight 검증을 수행하는 것이다. 특히 운영 컨테이너가 `0.9.0.2-1`인 동안에는 `0.9.0.2-3`과 이후 source 보강에 포함된 duplicate cleanup, startup FK 진단 강화, 일부 scan debt 회복 동작이 실제 운영 DB에는 아직 반영되지 않을 수 있다.
...
- public release: `0.9.0.2-3`
- source branch delta after public release: GDS 타입 처리 보강 1개 커밋
